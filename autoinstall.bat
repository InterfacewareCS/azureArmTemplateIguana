...
SET port=6543
SET RULE_NAME="Iguana Dashboard Port %port%"
SET port2=6544
SET RULE_NAME2="Iguana HTTP Port %port2%"
SET IguanaMainRepoLink="https://cf-us-west-1-iguana-templates.s3.us-west-1.amazonaws.com/IguanaMainRepo.zip"
SET IguanaEnvLink="https://cf-us-west-1-iguana-templates.s3.us-west-1.amazonaws.com/IguanaEnv.txt"
REM ###############################################################################################
...
echo 1.2 [Optional] Download and Extract IguanaMainRepo and IguanaEnv files
powershell -Command "(New-Object Net.WebClient).DownloadFile('%IguanaMainRepoLink%', '%downloadFolder%\IguanaMainRepo.zip')"
@TIMEOUT /t 1 /nobreak>nul
powershell.exe -nologo -noprofile -command "& { Add-Type -A 'System.IO.Compression.FileSystem'; [IO.Compression.ZipFile]::ExtractToDirectory('%downloadFolder%\IguanaMainRepo.zip', '%downloadFolder%'); }"
@TIMEOUT /t 1 /nobreak>nul 
powershell -Command "(New-Object Net.WebClient).DownloadFile('%IguanaEnvLink%', '%downloadFolder%\IguanaEnv.txt')"
@TIMEOUT /t 1 /nobreak>nul
if exist %downloadFolder%\IguanaMainRepo move %downloadFolder%\IguanaMainRepo %wDir%\IguanaMainRepo
if exist %downloadFolder%\IguanaEnv.txt move %downloadFolder%\IguanaEnv.txt %wDir%
...
netsh advfirewall firewall add rule name=%RULE_NAME2% dir=in action=allow protocol=TCP localport=%port2%
